function simulate_reservoir(sys, maxStep, perturb, tripName)
    ...
    execute!(sim, IDA(), dtmax=maxStep, enable_progress_bar = true)
...
function nonlinear_mapping!(sys, busCap, ibrBus, trainSamples, totalPower, R, stateIndex, simStep, perturb, timeSim=false)
    ...
    res = pmap(x -> get_W_rbf(trainSamples[:,x], rbfInput, resLB, resUB, timeSim), 1:size(trainSamples, 2)) # Distribute computation among all available workers
...
function simSystem!(sys, params, ibrBus, busCap, totalPower, simStep, perturb, timeSim=false)
    ...
    Gf=params[1:4].*(1 .-params[5:8]) # Grid following %
    GF=params[1:4].*params[5:8] # Grid forming %
    
    change_zone_ibr_penetration!.(Ref(sys), GF, Gf, ibrBus, Ref(busCap)) # Change generation mix at each of the IBR nodes
    ...
    execute!(sim, IDA(), enable_progress_bar = true)
