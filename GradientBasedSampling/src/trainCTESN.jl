...
for i = 1:total_iters
    comp_time[i] = @elapsed newtrainSamples = QuasiMonteCarlo.sample(5, LB, UB, QuasiMonteCarlo.LatinHypercubeSample())[:, 4:5] # Generate quasi-random samples from interior of parameter space
...
CSV.write("results/qmc/predict_rmse.csv", DataFrame(rmse, :auto), header=false)
CSV.write("results/qmc/comp_time.csv", DataFrame(reshape(comp_time, total_iters, 1), :auto), header=false)